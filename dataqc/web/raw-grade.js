...
window.NodaGrade={mount(container,onUpdate=()=>{}){
 ...
 return {setRoot(value){if((value||'')!==root)window.QCReport?.close();generation++;request++;root=value||'';snapshot=null;dirty=false;container.hidden=!root;reportBar.hidden=!root;reportBar.querySelector('button').disabled=true;reportBar.querySelector('span').textContent='读取质检报告…';current.textContent='读取中…';container.querySelector('.noda-capture').textContent='—';container.querySelector('.noda-qc').textContent='—';message.textContent='';select.value='';button.disabled=true;refresh()},refresh,destroy(){clearInterval(timer);root='';reportBar.remove()}};
}};
