...
def load_pose_samples(pose_dir):
    ...
    for filename in os.listdir(pose_dir):
        ...
        try:
            ...
            continue
        with open(os.path.join(pose_dir, filename), "r") as file:
            data = json.load(file)
        samples.append(
            (
                timestamp,
...
def interpolate_pose_samples(samples, target_times):
    target_times = np.asarray(target_times, dtype=np.float64)
    if target_times.size == 0:
        return np.zeros((0, 6), dtype=np.float64)
    if not samples:
        return np.zeros((target_times.size, 6), dtype=np.float64)
    ...
    sample_times = np.asarray(unique_times, dtype=np.float64)
    sample_values = np.vstack(unique_values).astype(np.float64, copy=False)
    if sample_times.size == 1:
        return np.repeat(sample_values[:1], target_times.size, axis=0)

    output = np.zeros((target_times.size, 6), dtype=np.float64)
    for col in range(5):
        output[:, col] = np.interp(target_times, sample_times, sample_values[:, col])
    output[:, 5] = normalize_yaw(
        np.interp(target_times, sample_times, np.unwrap(sample_values[:, 5]))
    )
    return output


def initialise_color_camera_data(data_dict, camera_names, config_paths):
    """Preserve legacy calibration keys except for the known uncalibrated head view."""
    del config_paths  # Config files are still read later when trusted calibration exists.
...
class Operator:
    def __init__(self, args):
        ...
        self.masterSyncDir = os.path.join(self.episodeDir, "master_sync.txt")
    ...
    def load_master_sync_times(self) -> List[float]:
        if not os.path.exists(self.masterSyncDir):
            return []
        times = []
        time_interval = int(self.args.timeInterval)
        with open(self.masterSyncDir, 'r') as lines:
    ...
    def process(self):
        error = False
        data_dict = {}
        initialise_color_camera_data(
            data_dict,
            self.args.cameraColorNames,
            self.cameraColorConfigDirs,
        )
            ...
            with open(self.armJointStateSyncDirs[i], 'r') as lines:
                ...
                for t, line in enumerate(lines):
                    ...
                    with open(os.path.join(self.armJointStateDirs[i], line), 'r') as file:
                        data = json.load(file)
                        position = np.array(data['position'])
                        if master_arm_gripper_mm and position.size > 6:
                            position[6] /= 1000
                        if position.size == 7:
            ...
            with open(self.liftMotorSyncDirs[i], 'r') as lines:
                ...
                for t, line in enumerate(lines):
                    ...
                    with open(os.path.join(self.liftMotorDirs[i], line), 'r') as file:
                        data = json.load(file)
                        data_dict[f'lift/motor/{self.args.liftMotorNames[i]}'].append(
                            data.get('backHeight', data.get('back_height', 0.0))
                        )
                    count += 1
                if size_count == 0:
                    size_count = count
        data_dict['size'] = size_count
        master_sync_times = self.load_master_sync_times()
        if master_sync_times:
            if len(master_sync_times) < size_count:
                print(
                    f"Warning: master_sync.txt has {len(master_sync_times)} timestamps, "
                    f"but synced data has {size_count} frames"
                )
            data_dict[f'timestamp'] = master_sync_times[:size_count]
        target_times = data_dict[f'timestamp'][:size_count]
        for i, name in enumerate(self.args.localizationPoseNames):
            if is_optional_base_pose_name(name):
                samples = load_pose_samples(self.localizationPoseDirs[i])
                if samples:
                    data_dict[f'localization/pose/{name}'] = interpolate_pose_samples(
                        samples,
                        target_times,
                    )
        if True:  # not error:
