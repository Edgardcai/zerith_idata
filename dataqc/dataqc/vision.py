...
safe_trim_ids 从报告静止候选中选择可删除冗余等待的候选编号；只允许不承载关键接触、抽取、切换或必要保持的静止区间。删除时程序会保留端点并再次检查接缝和全字段同步；无法确认则不选。"""
    prompt += "\n必须结合 Action 和 State：双手任务左爪和右爪各闭合一次且反馈响应对应，阶段1只有左爪闭合，阶段2只有右爪闭合。单手任务只要求该手一次。根据画面核对闭合、持物和切换阶段，不能改阶段边界掩盖错误动作。数值硬失败不得被视觉通过覆盖。腰部/头部均值与Q01/Q99均为±0.02 rad，仅作预警；升降柱根据本条记录的目标高度；无独立目标时只检查首帧指令高度保持，Action/State逐帧容差±0.02 m，不从目录推断。必须在分级理由中说明这些报告中的异常。"
