...
def refresh_paused_run(rid, progress=lambda _: None):
    ...
    counts = {'B': 0, 'F': 0, 'pending': 0, 'incomplete': 0}
    for ep in db.episodes(rid):
        ...
        try:
            ...
            grade = 'F' if fatal else ('B' if warnings else None)
            status = 'rejected' if fatal else 'queued'
            reason = ('判 F：' + failure_reason(fatal)) if fatal else ('；'.join(warnings) or '数值检查完成') + '；待 Luna 视觉核对'
            ...
            db.update('episodes', ep['id'], status='incomplete', grade=None, reason=str(exc))
            counts['incomplete'] += 1
        progress(dict(episode=Path(ep['root']).name, counts=counts))
    db.update('runs', rid, config=cfg, error='', updated=time.time(), phase='新规则数值复检完成 · 已暂停，继续后使用 Luna')
    return counts
