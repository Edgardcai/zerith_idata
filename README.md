...
> **8090 网站最新更新：2026-09-15**。当前部署地址为 `http://192.168.10.49:8090/`。网站已更新日期／场景目录、采集状态与时序提示、按 UUID 定位改名和归档录像、三路同步播放，以及排除已放弃记录的保留数量统计。列表及 episode 编号不变。网站 67 项自动测试通过，真实录像同步播放及失败重试已验证；本次未启动机器人录制。使用与测试说明见 [collection_web/README.md](collection_web/README.md)。以下旧版说明和测试结果为历史记录。

> **最新备份：2026-09-09**。当前采用默认关闭、网站可选的深度采集，默认两阶段、按目录分组，并修复写入阻塞及回放时间轴。最近实采已接近正常组，但最新版仍待新录制与实机回放验收。请先阅读 [最新备份说明与验证范围](docs/collection_backup_20260909/README.md)；下方 1.0 内容为历史基线，冲突处以最新说明为准。
