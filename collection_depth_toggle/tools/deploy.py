...
boot_text = '[Service]\nExecStart=\nExecStart=/usr/bin/python3 /home/robot/collection_depth_toggle/tools/boot.py\n'
...
for path, text in [(boot_override, boot_text), (user_override, user_text)]:
    previous = text.replace('/home/robot/collection_depth_toggle', '/home/robot/collection_rgb_only')
    if path.exists() and path.read_text() not in (text, previous):
...
print('Selectable depth collection deployed; default off and RGB preview retained.', flush=True)
