...
async function api(path,body,retryToken=true){
 const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),body?60000:20000);
 try{
  const response=await fetch(path,{method:body?'POST':'GET',headers:body?{'Content-Type':'application/json','X-Collection-Token':token}:{},body:body?JSON.stringify(body):undefined,cache:'no-store',signal:controller.signal});
  const result=await response.json();
  // Only retry an explicit pre-dispatch token rejection, never an uncertain POST.
  if(body&&retryToken&&response.status===403&&result.code==='TOKEN_EXPIRED'){
   token=(await api('/api/bootstrap')).csrf;return await api(path,body,false);
  }
  if(!response.ok)throw Error(result.error||'请求失败');return result;
 }catch(error){if(error.name==='AbortError')throw Error(body?'请求超时，请刷新确认操作结果后再操作':'连接超时，正在等待恢复');throw error;}
...
function episodeRowsHTML(rows){return rows.map(r=>{
 ...
 const warnings=[...(detail.warnings?.length?detail.warnings:detail.error?[detail.error]:[]),...(detail.delete_error?['放弃未完成：'+detail.delete_error]:[])];
 const location=r.location_status==='located'?`目录 ${r.current_name}`:({discarded:'已放弃，文件已删除',discarding:'正在放弃',unavailable:'目录待核对',pending:'尚未保存'})[r.location_status]||'';
 return `<tr><td class="episode-name" title="${esc(r.current_path||r.location_error||r.uuid)}"><span>采集 ${esc(r.seq)}</span><small class="episode-location">${esc(location)}</small></td><td><div class="grades" role="group" aria-label="${esc(r.name)} 评级">${['A','B','F'].map(g=>`<button class="grade ${r.grade===g?'selected':''}" data-id="${r.id}" data-grade="${g}" data-editable="${done?'1':'0'}" aria-label="${esc(r.name)} 评为 ${g}" aria-pressed="${r.grade===g}" ${done&&!busy?'':'disabled'}>${g}</button>`).join('')}</div></td><td><span class="badge ${done?'done':''}">${stateLabels[r.state]||esc(r.state)}</span></td><td>${duration(detail.duration_s??progress.elapsed_s)} · ${detail.frames??progress.frames??'—'} 帧</td><td class="quality">${warnings.length?`<details data-quality="${r.id}"><summary>${warnings.length} 项提示</summary><p>${warnings.map(esc).join('<br>')}</p></details>`:'—'}</td><td><div class="row-actions"><button class="text-button" data-copy-location="${r.id}" ${r.location_status==='located'?'':'disabled'}>复制路径</button><button class="text-button" data-review="${r.id}" ${done?'':'disabled'}>查看</button><button class="text-button discard" data-delete="${r.id}" ${done||r.state==='deleting'?'':'disabled'}>${r.state==='deleting'?'重试放弃':'放弃'}</button></div></td></tr>`;
 }).join('');}
...
let pollSequence=0;
async function poll(){const sequence=++pollSequence;try{const [status,groups]=await Promise.all([api('/api/status?compact=1'),api('/api/episode-groups')]);if(sequence!==pollSequence)return;snapshot=status;groupSummaries=groups.groups;render();await refreshOpenGroups();}catch(error){if(sequence!==pollSequence)return;$('connection').textContent='网站连接中断';$('connection').classList.remove('online');$('alert').hidden=false;$('alert').textContent=error.message;document.querySelectorAll('#start,#end,#check,#cameraToggle,#applyScene').forEach(el=>el.disabled=true);}}
...
$('episodes').onclick=e=>{const b=e.target.closest('button');if(!b||b.disabled)return;if(b.dataset.copyLocation)return action(async()=>{const info=await api('/api/episode-location/'+b.dataset.copyLocation);if(!info.current_path)throw Error(info.location_error||'当前没有可用目录');try{await navigator.clipboard.writeText(info.current_path);toast('实际目录已复制');}catch(_){const input=document.createElement('textarea');input.value=info.current_path;document.body.append(input);input.select();const copied=document.execCommand('copy');input.remove();if(!copied)throw Error('复制失败，请选中目录文字复制：'+info.current_path);toast('实际目录已复制');}});if(b.dataset.grade){const id=Number(b.dataset.id),grade=b.dataset.grade,path=b.closest('.dataset-group').dataset.path;return action(async()=>{await api('/api/episode/rate',{id,grade});await loadDataset(path,true);toast(`已评为 ${grade}`);});}if(b.dataset.delete){const id=Number(b.dataset.delete),path=b.closest('.dataset-group').dataset.path;return action(async()=>{if(!await confirm('放弃本条数据？','将删除本条 HDF5、视频及相关文件。','放弃并删除'))return;try{await api('/api/episode/delete',{id,confirm:'delete'});toast('本条数据已删除');}finally{await loadDataset(path,true);}});}if(b.dataset.review)reviewPlayer.open(b.dataset.review,b.closest('tr').querySelector('.episode-name').innerText.replace(/\n/g,' · '));};
