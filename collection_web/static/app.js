...
let token='', snapshot=null, busy=false, restored=false, cameraTimer=null, toastTimer=null, directoryTimer=null, directoryVersion=0, directoryPreview='', directoryError='', pendingParse=Promise.resolve();
...
function task(){return {task_name:$('prompt').value,task_id:$('taskId').value,subtask_num:$('subtasks').value,frequency:$('frequency').value,max_episode_time:$('maxTime').value,left:$('left').value,right:$('right').value,lift_height:$('liftHeight').value,record_depth:$('recordDepth').checked};}
...
function queueDirectory(){clearTimeout(directoryTimer);const version=++directoryVersion;directoryPreview='';directoryError='';renderDirectory();directoryTimer=setTimeout(async()=>{try{const result=await api('/api/task/directory',task());if(version!==directoryVersion)return;directoryPreview=result.dataset;directoryError='';}catch(error){if(version!==directoryVersion)return;directoryPreview='';directoryError=error.message;}renderDirectory();if(snapshot){renderEpisodeGroups();refreshOpenGroups();}},200);}
...
function renderAvailability(){const phase=snapshot?.collection?.phase||'idle';const editable=['idle','closed','rejected'].includes(phase);$('start').disabled=busy||!editable;$('end').disabled=busy||editable||Boolean(snapshot?.collection?.current);$('check').disabled=busy;$('cameraToggle').disabled=busy;document.querySelectorAll('#taskForm input,#taskForm textarea').forEach(el=>el.disabled=busy||!editable);$('recordDepth').disabled=busy||!editable||!depthSupported;}
...
function episodeRowsHTML(rows){return rows.map(r=>{const done=r.state==='completed',detail=r.detail||{},progress=r.progress||{};const warning=detail.warnings?.join(' · ')||detail.error||'';return `<tr><td>${esc(r.name)}<br><span class="muted">${esc(r.uuid.slice(0,8))}</span></td><td><span class="badge ${done?'done':''}">${stateLabels[r.state]||esc(r.state)}</span></td><td>${detail.frames??progress.frames??'—'} 帧<br><span class="muted">${duration(detail.duration_s??progress.elapsed_s)}</span>${detail.total_steps?`<br><span class="muted">阶段 ${detail.completed_steps||0} / ${detail.total_steps}</span>`:''}</td><td class="quality ${warning?'warn':''}">${esc(warning|| (done?'结构检查通过':'—'))}</td><td><div class="grades">${['A','B','F'].map(g=>`<button class="grade ${r.grade===g?'selected':''}" data-id="${r.id}" data-grade="${g}" ${done?'':'disabled'}>${g}</button>`).join('')}</div></td><td><div class="row-actions"><button class="text-button" data-review="${r.id}" ${done?'':'disabled'}>查看</button><button class="text-button discard" data-delete="${r.id}" ${done?'':'disabled'}>放弃</button></div></td></tr>`;}).join('');}

const groupNodes=new Map(),datasetRows=new Map(),datasetRequests=new Map();
...
function groupCounts(group){return `已保存 ${group.completed||0} · A ${group.A||0} · B ${group.B||0} · F ${group.F||0} · 已放弃 ${group.deleted||0}`;}
...
function renderEpisodeGroups(){
 ...
 for(const [path,node] of groupNodes)if(!paths.has(path)){node.remove();groupNodes.delete(path);datasetRows.delete(path);}
 for(const group of groups){
  ...
  node.querySelector('.dataset-total').textContent=`${group.total||0} 条记录`;
 ...
 $('gradeCounts').textContent=current?`当前目录 · ${groupCounts(current)}`:'按保存目录分组，展开查看记录';
 $('episodeCount').textContent=current?.completed||0;
 $('empty').hidden=groups.length>0;
}
async function loadDataset(path){
 if(datasetRequests.has(path))return datasetRequests.get(path);
 const request=(async()=>{try{
  const rows=(await api('/api/episodes?dataset='+encodeURIComponent(path))).episodes;
  datasetRows.set(path,rows);const node=groupNodes.get(path);if(node)groupBody(node);
...
function renderStages(){
 ...
 $('stagePlan').innerHTML=Array.from({length:total},(_,index)=>{
  const title=total===2?(index===0?`左手抓取${left?' '+left:''}`:`保持左手持物，右手抓取${right?' '+right:''}`):`完成阶段 ${index+1} 的动作`;
  const state=index<completed?'complete':recording&&index===completed?'active':'';
  return `<li class="${state}"><span class="stage-number">${index<completed?'✓':index+1}</span><div><strong>阶段 ${index+1}</strong><span>${esc(title)}</span><small>${index+1<total?`短按 Y → 阶段 ${index+2}`:'短按 Y → 结束本条并保存'}</small></div></li>`;
 }).join('');
 $('stageHint').textContent=recording&&completed<total?`正在第 ${Math.min(completed+1,total)} / ${total} 阶段：${completed+1<total?'短按一次 Y 切换下一阶段':'短按一次 Y 结束本条并保存'}。每次按下后请松开。`:(c?.current?'本条正在保存，阶段分界随数据一起保留。':'长按 Y 超过 2 秒开始本条；松开后，每短按一次 Y 完成一个阶段，最后一次结束并保存。');
}
...
async function poll(){try{snapshot=await api('/api/status');groupSummaries=(await api('/api/episode-groups')).groups;render();await refreshOpenGroups();}catch(error){$('connection').textContent='网站连接中断';$('connection').classList.remove('online');$('alert').hidden=false;$('alert').textContent=error.message;document.querySelectorAll('#start,#end,#check,#cameraToggle').forEach(el=>el.disabled=true);}}
...
$('start').onclick=()=>action(async()=>{await pendingParse;const t=task();await api('/api/task/directory',t);if(!await confirm('启动采集？',`共 ${t.subtask_num} 个阶段，${t.record_depth?'记录彩色和深度图':'仅记录彩色图像'}，由 Meta Quest 控制每条录制。`,'启动'))return;await api('/api/session/start',t);restored=false;toast('采集配置已提交');});
...
$('teleopVoice').onclick=()=>{if(!('speechSynthesis' in window)){toast('当前浏览器不支持语音，页面仍显示标定状态');return;}teleopVoice=!teleopVoice;$('teleopVoice').textContent=teleopVoice?'关闭语音提示':'开启语音提示';if(teleopVoice)sayTeleop('语音提示已开启');else speechSynthesis.cancel();};
...
(async()=>{renderJoints([]);try{const boot=await api('/api/bootstrap');token=boot.csrf;depthSupported=boot.depth_recording_supported===true;$('prompt').value=boot.default_prompt;await parse();await poll();}catch(e){toast(e.message,true);}async function tick(){await poll();setTimeout(tick,800);}setTimeout(tick,800);})();
