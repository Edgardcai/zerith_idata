...
def compare(a, b):
    with h5py.File(a/'episode.hdf5', 'r') as left, h5py.File(b/'episode.hdf5', 'r') as right:
        ...
        assert names == other
        assert right.attrs['image_streams'] == 'color'
        assert not right.attrs['depth_recorded']
        for name in names:
...
# The acquisition adapter must not even request depth frames.
requested = []
def image_getter(camera, kind):
    requested.append((camera, kind))
    assert kind == 'color'
    return batch[0][0]['images'][camera]['color']
...
assert len(requested) == 3 and all(set(x) == {'color'} for x in images.values())
# Exercise the real camera dataclasses and initialization/reinitialization filter
# through a fake constructor, with hardware constructors still denied.
spec = RsRecorder.CameraSpec(name='test', serial='test', align_to='depth to color', desired_streams=[
    RsRecorder.StreamSpec(type='color', width=640, height=480, fps=30, fmt='bgr8'),
    RsRecorder.StreamSpec(type='depth', width=640, height=480, fps=30, fmt='z16')])
class FakeRecorder:
    def __init__(self, camera_specs=None): self.specs = camera_specs
    def reinit_camera(self, camera_name, streams=None): return streams
rgb.install_cameras(SimpleNamespace(ImageRecorder=FakeRecorder, logger=collect_data.logger))
recorder = FakeRecorder([spec])
assert [x.type for x in recorder.specs[0].desired_streams] == ['color']
assert recorder.specs[0].align_to is None and len(spec.desired_streams) == 2
assert [x.type for x in recorder.reinit_camera('test', spec.desired_streams)] == ['color']
worker = patch.find_code(collect_data.EpisodeDataCollector._capture_episode.__code__, 'save_worker')[0]
assert ('color',) in worker.co_consts and ('color','depth','ir1','ir2') not in worker.co_consts
with tempfile.TemporaryDirectory(prefix='storage-selftest-', dir=ROOT/'runtime') as directory:
    ...
    # No depth compression may occur even when the input carries depth images.
    context.zlib = SimpleNamespace(compress=denied)
    context.proxy.original = context.zlib
    sustained = write(wrapped_code, d/'sustained', batch*5, paced=True)
    stats = {'baseline_rgbd_ms': before, 'rgb_only_ms': after, 'frames': len(batch),
             'nondepth_datasets_and_encoded_images_identical': True, 'decoded_videos_identical': True,
             'depth_acquisition_and_storage_disabled': True, 'paced_30hz_frames': len(batch)*5,
