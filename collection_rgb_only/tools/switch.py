...
def main():
    ...
    with (ROOT/'runtime/switch.lock').open('a') as lock:
        ...
        if args.action == 'start':
            ...
            if old_fast:
                previous = Path('/home/robot/collection_storage_fast/runtime')
                ...
                if (not args.replace_fast or not previous_marker.get('passed') or
                        any(digest('/proc/%d/exe' % pid) != previous_marker['sha256'] for pid in old_fast)):
                    raise RuntimeError('仅允许替换已验证的 RGB-D 优化服务')
            old = pids('server')
            ...
            command = 'cd /opt/robot && /home/robot/collection_rgb_only/runtime/server_fast --live; exec sh'
        else:
        ...
        if args.action == 'start':
            ...
            fallback = ('/home/robot/collection_storage_fast/runtime/server_fast --live'
