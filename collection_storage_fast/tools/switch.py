...
def main():
    ...
    with (ROOT/'runtime/switch.lock').open('a') as lock:
        ...
        if args.action == 'start':
            ...
            if pids('server_fast'): raise RuntimeError('优化采集服务已经运行')
            old = pids('server')
            if not old and not args.recover_stopped:
            ...
            command = 'cd /opt/robot && /home/robot/collection_storage_fast/runtime/server_fast --live; exec sh'
        else:
        ...
        if args.action == 'start':
            ...
            launch('cd /opt/robot && ./server; exec sh')
